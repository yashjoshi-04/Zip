...
public:
			...
			threads_.emplace_back([this] {
				while (true) {
					...
					// threads can perform enqueue tasks
					{
						...
						task = std::move(tasks_.front()); // Corrected to std::move
						tasks_.pop();
					}
					task();
				}
	...
	~ThreadPool() {
        ...
        for (auto& thread : threads_) {
            if (thread.joinable()) { // Check if joinable before joining
                thread.join();
            }
        }
    }
    ...
    template<class F> // Use template for more flexible task types
    void enqueue(F&& task_func)  {
        {
            std::unique_lock<std::mutex> lock(queue_mutex_);
            tasks_.push(std::function<void()>(std::forward<F>(task_func)));
        }
        cv_.notify_one();
    }
